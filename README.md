# A Node.js zimstake Client!

node-zimstake is a zimstake client for Node.js. It is a fork of the excellent Kapitalize Bitcoin Client (now removed from GitHub) intended for use with zimstake. The purpose of this repository is:

* Provide a one-stop resource for the Node.js developer to get started with zimstake integration.
* Prevent would-be zimstake web developers worrying whether a Bitcoin client will work out of the box.
* Promote Node.js development of zimstake web apps.
* Identify and address any incompatibilities with the zimstake and Bitcoin APIs that exist now and/or in the future.

## Dependencies

You'll need a running instance of [zimstaked](https://github.com/CryptoGir/Zimstake.git) to connect with.

Then, install the node-zimstake NPM package.

`npm install node-zimstake`

## Examples

Some code examples follow below, but for more complete examples, see [these snippets](https://github.com/brotchie/zimstake-code-snippets), or this [wallet app](https://github.com/countable/dogelet) which was created to to test this module.

```js
var zimstake = require('node-zimstake')()

zimstake.auth('myusername', 'mypassword')

zimstake.getDifficulty(function() {
...
```js
zimstake.getNewAddress(function(err, address) {
...
```js
zimstake.exec('getNewAddress')

zimstake.exec('getbalance', function(err, balance) {
...
<td> <b>Currently only available on testnet</b> Add a nrequired-to-sign multisignature address to the wallet. Each key is a zimstake address or hex-encoded public key. If [account] is specified, assign address to [account]. </td>
<td> N
</td></tr>
<tr>
<td> backupwallet </td>
...
<tr>
<td> dumpprivkey </td>
<td> [zimstakeaddress] </td>
<td> Reveals the private key corresponding to <zimstakeaddress< </td>
<td> Y
</td></tr>
<tr>
<td> encryptwallet </td>
...
<td> [account] </td>
<td> Returns the current zimstake address for receiving payments to this account. </td>
<td> N
</td></tr>
<tr>
...
<td> getgenerate </td>
<td> </td>
<td> Returns true or false whether zimstaked is currently generating hashes </td>
<td> N
</td></tr>
<tr>
...
<td> getnewaddress </td>
<td> [account] </td>
<td> Returns a new zimstake address for receiving payments.  If [account] is specified (recommended), it is added to the address book so payments received with the address will be credited to [account]. </td>
<td> N
</td></tr>
<tr>
...
<td> [zimstakeaddress] [minconf=1] </td>
<td> Returns the total amount received by <zimstakeaddress< in transactions with at least [minconf] confirmations. While some might consider this obvious, value reported by this only considers *receiving* transactions. It does not check payments that have been made *from* this address. In other words, this is not "getaddressbalance". Works only for addresses in the local wallet, external addresses will always show 0. </td>
<td> N
</td></tr>
<tr>
...
<p>To get a list of accounts on the system, execute zimstaked listreceivedbyaddress 0 true
</p>
</td>
<td> N
</td></tr>
...
<tr>
<td> sendfrom </td>
<td> [fromaccount] [tozimstakeaddress] [amount] [minconf=1] [comment] [comment-to] </td>
...
<td> stop </td>
<td> </td>
<td> Stop zimstake server. </td>
<td> N
</td></tr>
<tr>
<td> validateaddress </td>
<td> [zimstakeaddress] </td>
<td> Return information about [zimstakeaddress]. </td>
<td> N
</td></tr>
<tr>
<td> verifymessage </td>
<td> [zimstakeaddress] [signature] [message] </td>
...
You may pass an optional function `passphrasecallback` to the `node-zimstake` initialization function to manage wallet unlocks. `passphrasecallback` should be a function accepting three arguments:
...
By default `zimstaked` exposes its JSON-RPC interface via HTTP; that is, all RPC commands are transmitted in plain text across the network! To secure the JSON-RPC channel you can supply `zimstaked` with a self-signed SSL certificate and an associated private key to enable HTTPS. For example, in your `zimstake.conf`:

    rpcssl=1
    rpcsslcertificatechainfile=/etc/ssl/certs/zimstaked.crt
    rpcsslprivatekeyfile=/etc/ssl/private/zimstaked.pem

In order to securely access an SSL encrypted JSON-RPC interface you need a copy of the self-signed certificate from the server: in this case `zimstaked.crt`. Pass your self-signed certificate in the `ca` option and set `https: true` and node-zimstake is secured!
    
```js
var fs = require('fs')

var ca = fs.readFileSync('zimstaked.crt')
...
nodeunit test/test-node-zimstake.js
```

## Bounties

[zimstake](http://www.zimstake.com) donation address is DE4isu3m2RBma7nGEwnaX8cu4Y2m2J2g8Q

Donations in [zimstake](http://www.zimstake.com) will be used for bounties. The first bounty will be awarded for creating a unit test suite. As a side note: I encourage all GitHub repository owners to post a donation address so their community can easily support development financially. If you accept donations at your repository, [I'll send you a reward!](http://www.reddit.com/r/zimstakedev/comments/203ojs/add_a_zimstake_donation_link_in_your_github_repo/)
